...
- [x] 8.1 Write the ADR for design decisions D1 + D2 (lazy deadline enforcement; single-lock atomic turn core) under `docs/adr/` → `docs/adr/0002-lazy-turn-deadline-and-atomic-turn-core.md`.
- [x] 8.2 Full browser e2e (backend in degraded mode, worktree dev server, 2 real registered players): **happy path** — A submitted boost 2 via the real UI, entered "Waiting for other players" with a live MM:SS countdown (04:31), B submitted via API ⇒ A's UI advanced within one poll (lap 2/3, countdown re-armed 04:46, boost ×2→×1); **timeout path** — A submitted, B absent, expiry auto-played B and resolved the turn (history recorded); **fully absent** — races advanced turn-by-turn on polls alone with both players auto-played (real cards consumed); **resume** — twice-auto-played B's manual submit accepted normally; **solo regression** — full 5-turn solo race to "Race completed! You finished in position 3" with the countdown never rendering (`sawCountdown: false`) and zero console errors. Evidence is DOM-asserted (browser-pane screenshots unavailable in this environment — renderer timeouts). Also caught+fixed during e2e: the preview server was initially serving the bare checkout instead of the worktree (repo's known concurrent-session hazard).
- [x] 8.3 Run both verify gates once more plus `openspec validate --all --strict`, then the review gate and resolve any BLOCK before opening the PR into `dev`.

> Review gate: **PASS** — verdict artifact at `docs/reviews/feat-multiplayer-turn-sync-1.md`. Three judges (spec-conformance: 19/19 scenarios PASS; correctness: 8 recall-biased finder angles; security: CLEAN) surfaced 8 blocking-grade findings; all fixed in-branch (commit d2a1d9c) with tests-first where behavior changed, gates re-run green (backend 154, frontend 119, specs strict-valid), and the pit fix re-verified live in the browser.
