...
Read just enough of the codebase (Grep/Glob, relevant `openspec/specs/`
current-truth capability specs, `docs/`) to ground the conversation. Don't boil
the ocean — locate, skim, orient.
...
## Suggested next step
- /opsx:propose to turn this into an `openspec/changes/<change>/` proposal (specs deltas + tasks).
```
