...
describe('BoostControlPanel', () => {
  ...
  it('shows validate turn button when boost is selected', () => {
    ...
    expect(screen.getByRole('button', { name: 'Validate turn with selected boost' })).toBeInTheDocument();
  });

  it('disables validate button when no boost is selected', () => {
    render(<BoostControlPanel {...defaultProps} selectedBoost={null} />);
    
    const validateButton = screen.getByRole('button', { name: 'Validate turn with selected boost' });
  ...
  it('shows confirmation dialog when validate button is clicked', () => {
    ...
    expect(screen.getByText('You are about to validate your turn with boost value:')).toBeInTheDocument();
  });

  it('calls onValidateTurn when confirmation is clicked', () => {
    const onValidateTurn = vi.fn();
    render(<BoostControlPanel {...defaultProps} selectedBoost={2} onValidateTurn={onValidateTurn} />);
