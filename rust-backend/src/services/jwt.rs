...
impl JwtService {
    ...
    pub fn generate_access_token(&self, user: &Player) -> Result<String, JwtError> {
        let now = Utc::now();
        let exp = now
            + Duration::from_std(self.config.access_token_expiry)
    ...
    pub fn generate_refresh_token(&self, user: &Player) -> Result<String, JwtError> {
        let now = Utc::now();
        let exp = now
            + Duration::from_std(self.config.refresh_token_expiry)
                .map_err(|e| JwtError::TokenGeneration(e.to_string()))?;
...
mod tests {
    ...
    use crate::domain::{
        ...
        Pilot, PilotClass, PilotName, PilotPerformance, PilotRarity, PilotSkills, TeamName,
    };
