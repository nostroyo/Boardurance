...
use crate::domain::{
    Body, BodyName, Car, CarName, ComponentRarity, Email, Engine, EngineName, Password, Pilot,
...
> {
    ...
    let token_pair = app_state
        ...
        .map_err(|e| {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ResponseJson(json!({"error": "Failed to generate authentication tokens"})),
            )
        })?;
    ...
    let access_claims = app_state
        ...
        .map_err(|e| {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ResponseJson(json!({"error": "Token generation error"})),
            )
        })?;
    ...
    app_state
        .session_manager
        .create_session(player.uuid, access_claims.jti.clone(), session_metadata)
        .await
        .map_err(|e| {
            tracing::error!("Failed to create session: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ResponseJson(json!({"error": "Session creation failed"})),
            )
        })?;
...
> {
    ...
    let is_valid = user.verify_password(&password).map_err(|e| {
        ...
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            ResponseJson(json!({"error": "Authentication error"})),
        )
    })?;
    ...
    app_state
        .session_manager
        .create_session(user.uuid, access_claims.jti.clone(), session_metadata)
        .await
        .map_err(|e| {
            tracing::error!("Failed to create session: {}", e);
            (
...
> {
    ...
    if let Some(token) = token {
        ...
        if let Err(e) = app_state
            .session_manager
            .invalidate_session(&token, "user_logout")
            .await
        {
...
> {
    ...
    let refresh_token = extract_refresh_token_from_headers(&headers).ok_or_else(|| {
        (
            StatusCode::UNAUTHORIZED,
            ResponseJson(json!({"error": "Refresh token not found"})),
        )
    })?;
    ...
    let claims = app_state
        ...
        .map_err(|_| {
            (
                StatusCode::UNAUTHORIZED,
                ResponseJson(json!({"error": "Invalid refresh token"})),
            )
        })?;
    ...
    if is_blacklisted {
        return Err((
            StatusCode::UNAUTHORIZED,
            ResponseJson(json!({"error": "Token has been revoked"})),
        ));
    }
    ...
    let user_uuid = uuid::Uuid::parse_str(&claims.sub).map_err(|_| {
        (
            StatusCode::UNAUTHORIZED,
            ResponseJson(json!({"error": "Invalid token"})),
        )
    })?;
    ...
    let user = app_state
        ...
        .ok_or_else(|| {
            (
                StatusCode::UNAUTHORIZED,
                ResponseJson(json!({"error": "User not found"})),
            )
        })?;
    ...
    let new_access_token = app_state
        ...
        .map_err(|e| {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ResponseJson(json!({"error": "Token generation failed"})),
            )
        })?;
    ...
    if let Err(e) = app_state
        .session_manager
        .invalidate_session(&claims.jti, "token_refresh")
        .await
    {
    ...
    let new_claims = app_state
        ...
        .map_err(|e| {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                ResponseJson(json!({"error": "Token validation error"})),
            )
        })?;
    ...
    if let Err(e) = app_state
        .session_manager
        .create_session(user.uuid, new_claims.jti, session_metadata)
        .await
    {
