...
> [!NOTE]
> **`main` is the last release; `dev` is where the work happens.** The BMAD
> tooling this README describes lives on `dev` and has not been released to
> `main` yet, so the few links to it point at the `dev` branch. `main`'s own
> `CLAUDE.md` still documents the older OpenSpec process until the next release.
> For the current state of the project, browse
> [the `dev` branch](https://github.com/nostroyo/Boardurance/tree/dev).
...
| [`_bmad-output/`](https://github.com/nostroyo/Boardurance/tree/dev/_bmad-output) | **The spec source of truth** — where BMAD writes planning artifacts, epics/stories, review & test artifacts. Mostly empty so far: BMAD landed recently, so today it holds `project-context.md` and a little implementation output. *(on `dev`)* |
| [`_bmad/`](https://github.com/nostroyo/Boardurance/tree/dev/_bmad) | BMAD installation and config (durable overrides in `_bmad/custom/`). *(on `dev`)* |
...
[`docs/BMAD_WORKFLOW_GUIDE.md`](https://github.com/nostroyo/Boardurance/blob/dev/docs/BMAD_WORKFLOW_GUIDE.md)
(on `dev`); the rules live in
[`CLAUDE.md`](CLAUDE.md).
...
| I want to… | Read |
|------------|------|
| Start a feature | [`docs/BMAD_WORKFLOW_GUIDE.md`](https://github.com/nostroyo/Boardurance/blob/dev/docs/BMAD_WORKFLOW_GUIDE.md) (on `dev`) — or just invoke `bmad-help` |
| Know the working rules | [`CLAUDE.md`](CLAUDE.md) (+ per-tier `rust-backend/CLAUDE.md`, `empty-project/CLAUDE.md`) |
| See current plans, stories, reviews | [`_bmad-output/`](https://github.com/nostroyo/Boardurance/tree/dev/_bmad-output) (on `dev`) |
